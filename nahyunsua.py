...
# ===========================
# 4. 지도 생성 함수 (파랑~빨강, Top5 강조, hover에 지역+값)
# ===========================
def create_map(df_full, df_coord, selected_year, top5_year=2050):
    """
    선택 연도 기준으로 지역별 pred 값을 지도에 표시.
    - 파랑(낮음) ~ 빨강(높음)
    - Top5 지역은 굵은 원 + 테두리 강조
    - 마커 클릭으로 지역 선택하는 기능은 제거
    - 마우스 오버(tooltip) 시 '지명 + 값'만 표시
    - 값이 높을수록(더 붉을수록) 원의 반지름이 커짐
    """
    ...
    for _, row in df_map.iterrows():
        ...
        # Top5는 테두리만 조금 더 두껍게 + 검은색
        if row["region"] in top5_regions:
            weight = 3
            border_color = "black"

        tooltip_text = f"{row['region']} : {row['value']:.2f} tCO₂eq/km²"
        ...
        CircleMarker(
            ...
            fill_opacity=0.9,
            weight=weight,
            tooltip=tooltip_text,      # 마우스 오버 시 지명 + 값 표시
        ).add_to(m)
...
with tab3:
    ...
    st.markdown(
        ...
        **MAE(Mean Absolute Error, 평균 절대 오차)** 는  
        모델 예측값과 실제 값의 차이를 **절대값**으로 만든 뒤, 그 값들의 **평균**을 낸 지표입니다.
        ...
        **해석을 위한 대략적인 기준(예시)**  
        ...
        - MAE **≥ 15** : 실제 값과 차이가 꽤 큰 편  
          → 데이터 보완(추가 변수, 이상치 정리) 또는 모델 구조 개선(XGBoost 파라미터 튜닝 등)이 필요할 수 있습니다.
        """
    )

    # 지역 이름(지명)을 ㄱㄴㄷ 순서(알파벳 오름차순)로 정렬해서 표/그래프 둘 다 사용
    df_mae_sorted = df_mae.sort_values("region")

    st.markdown("### 지역별 MAE 표 (ㄱㄴㄷ 순 정렬)")
    st.dataframe(
        df_mae_sorted.rename(columns={"region": "Region", "MAE": "MAE (tCO₂eq/km²)"}),
        use_container_width=True,
    )

    st.markdown("### 지역별 MAE 막대 그래프")
    st.bar_chart(df_mae_sorted.set_index("region")["MAE"])

    csv_mae = df_mae_sorted.to_csv(index=False).encode("utf-8-sig")
    st.download_button(
